...
def hitran_convention_lorentz(Nu,Gamma0,Delta0,WnGrid,YRosen=0.,Sw=1.):
    """Adapt the HAPI 1.3 Lorentz shift sign to HITRAN's nu+delta*p.

    This is explicitly a reference-check adapter, not a change to the
    physical forward model or to the installed third-party package.
    """
    import hapi
    return hapi.PROFILE_LORENTZ(Nu,Gamma0,-Delta0,WnGrid,YRosen,Sw)


def main():
    parser=argparse.ArgumentParser()
    parser.add_argument('--resume-integration',action='store_true',help='Reuse hash-verified completed integration arrays; rerun all reference checks.')
    ...
    if args.resume_integration:
        checkpoint=json.loads((out/'integration_checkpoint.json').read_text())
        assert checkpoint['input_sha256']==digest(line_file)
        for name,expected in checkpoint['code_hashes'].items():
            assert digest(ROOT/name.replace('\\','/'))==expected,name
        for name,expected in checkpoint['outputs'].items():
            assert digest(out/name)==expected,name
        current=dict(np.load(out/'refracted45_inputs.npz'))
        rows=pd.read_csv(out/'elevation_geometry.csv').to_dict('records')
        convergence=pd.read_csv(out/'quadrature_convergence.csv').to_dict('records')
        print('Reusing verified integration checkpoint; rerunning reference validation.',flush=True)
    else:
        ...
        checkpoint=dict(input_sha256=digest(line_file),integration_source_sha256=digest(Path(__file__)),
            code_hashes={str(path.relative_to(ROOT)):digest(path) for path in [ROOT/'src/thz_isac/slant_path.py',ROOT/'src/thz_isac/stratified_absorption.py',ROOT/'src/thz_isac/physical_spectroscopy.py']},
            outputs={name:digest(out/name) for name in ['refracted45_inputs.npz','reference_path.csv','elevation_geometry.csv','quadrature_convergence.csv','line_shape_comparison.csv']})
        write_json(out/'integration_checkpoint.json',checkpoint)
    print('Checking multiple pressure/temperature/self-broadening states against HAPI.',flush=True)
    import hapi
    source_path=Path(inspect.getsourcefile(hapi.PROFILE_LORENTZ))
    grid=np.array([9.98,10.02])
    raw=hapi.PROFILE_LORENTZ(10.,.1,.02,grid)
    adapted=hitran_convention_lorentz(10.,.1,.02,grid)
    write_json(out/'hapi_pressure_shift_sign.json',dict(
        package_source_sha256=digest(source_path),package_source_file=source_path.name,
        expected_center_cm_1=10.02,unmodified_hapi_lorentz_center_on_probe_grid_cm_1=float(grid[raw.argmax()]),
        adapted_center_on_probe_grid_cm_1=float(grid[adapted.argmax()]),
        reference='https://hitran.org/docs/definitions-and-units/',
        note='HITRAN nu*=nu+delta*p. HAPI 1.3 PROFILE_LORENTZ uses WnGrid+Delta0-Nu. The comparison adapter negates Delta0; the research forward model is unchanged.'))
    assert grid[adapted.argmax()]==10.02
    validation=[]
        ...
        for altitude in (0,6000,18000):
            ...
            for name,mid in [('H2CO',20),('CH3OH',39),('CH3CN',41),('CO',5),('O3',3),('SO2',9),('NO2',10),('H2O',1),('O2',7)]:
                ...
                for profile in ('voigt','lorentz'):
                    ...
                    routine=hapi.absorptionCoefficient_Voigt if profile=='voigt' else hapi.absorptionCoefficient_Generic
                    profile_options={} if profile=='voigt' else dict(profile=hitran_convention_lorentz,calcpars=hapi.calculateProfileParametersLorentz)
                    _,external=routine(**profile_options,Components=((mid,1),),SourceTables=name+'_main_0_3000GHz',
                    ...
                    validation.append(dict(altitude_m=altitude,molecule=name,profile=profile,self_fraction=q,
                                           ...
                                           reference='HAPI Voigt' if profile=='voigt' else 'HAPI Lorentz with documented Delta0 sign adapter'))
    ...
    pd.DataFrame(dict(frequency_ghz=f,itu_refracted_db=current['background_db'],
                    hitran_refracted_db=current['hitran_background_db'])).to_csv(out/'background_models.csv',index=False)
