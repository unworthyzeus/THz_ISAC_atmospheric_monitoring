...
Run from this project directory with Python 3.12; the provenance helper is now included in `scripts/repair_support.py`. The exact versions and input/output hashes are in the [execution manifest](../results/repair_correlated/manifest.json). This experiment uses the CPU. Parent-process RSS is sampled every 0.1 seconds; it is not an exact process peak, and the online EM worker memory is unavailable.
